...
- **No arbitrary `sign(bytes)`** — proof execution is bound to task/intent fields; connector crypto remains purpose-bound in `spec/wit/connector/world.wit`.
- **Ratchet shrink only** — `restrict` accepts a digest of a proposed capability subset; widening is rejected by the host engine.

## Task world (`spec/wit/task/world.wit`)
...
## Proof world (`spec/wit/proof/world.wit`)
...
## Mediation world (`spec/wit/mediation/world.wit`)
...
`spec/wit/host/world.wit` remains at `@1.0.0` with **comment-only** optional import notes. Existing session/invoke exports are unchanged.
