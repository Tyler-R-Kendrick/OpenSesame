...
`docs/agents/vault-key-protection-age-sops-prompt.md` § Required deliverables:
...
| C13 formats-interop core | `docs/agents/vault-key-protection-age-sops-prompt.md` § C13 | INTEROP | every `pnpm verify:sops-*` above | passed | this ledger | sops metadata edited-by-hand outside the engine is out of scope |
