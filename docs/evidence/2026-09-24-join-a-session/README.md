...
After: a 6-step rail (Where, Approval, Verify, Offer, Code, Joined), the
deployment's own endpoint prefilled, the invite field, one `.go`. Nothing
leaves the page on this rung.
...
After: the invite field goes and the rail becomes Where, Approval, Verify,
Session, Joined.
...
| screen | front door | join ceremony, invite filled |
...
covered by `apps/pages/src/screens/JoinScreen.test.tsx` and
`packages/app-core/src/lib/join/*.test.ts`, which walk approval → verify →
look-up → per-item choice → code → joined, and the open road to a pending
answer. The shared-origin refusal ("This address cannot finish a join", no
