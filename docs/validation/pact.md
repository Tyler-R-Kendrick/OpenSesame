...
| Attachment external tier | folder and logical paths reject traversal for every shape tried; digests are strict hex | a chunk whose body does not hash to its claimed digest is refused; a provider with no uploader is refused at configuration time; credentialed uploads never follow a redirect | replicate with no target configured names the fix rather than failing opaquely; an upload failure records against the target without disturbing configuration | target view carries no token material; OpenAPI documents all five routes; body limits are a compile-time assertion |
...
`packages/mcp-client/src/pact.test.ts`,
