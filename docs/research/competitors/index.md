...
| Varlock | Peer / compatibility | `.env.schema` + credential proxy | — (see [docs/reference/reuse.md](../../reference/reuse.md)) |
| Enpass | **Craft bar (vault sync)** — [ADR 0140](../../adr/0140-tailnet-vault-sync.md) | Password manager that syncs through the person's own storage (cloud, WebDAV, folder, desktop Wi-Fi Sync Server) with a client-side merge; model for the tailnet drive | [`enpass.md`](enpass.md) |
...
[docs/reference/reuse.md](../../reference/reuse.md)). Serving another vendor's clients is only ever
...
Pattern for new entries: add `docs/research/competitors/<slug>.md` with overview →
...
and on-disk surface needed for comparison. See [docs/reference/reuse.md](../../reference/reuse.md) for
