...
3. **Enabled registration types:** `anonymous` and `service_auth`. Provider `identity_assertion` (ID-JAG) is enabled only behind explicit agent-provider trust ([ADR 0124](0124-agent-auth-provider-id-jag.md)). SET events remain disabled and unadvertised.
