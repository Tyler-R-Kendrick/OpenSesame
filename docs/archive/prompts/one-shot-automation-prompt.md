...
**OWNS:** `ops/routines/**`, `tools/security/claude-review-checklist.md`,
...
4. `ops/routines/pr-security-review.md` — an on-demand deep-review prompt for a
   PR number: apply `tools/security/claude-review-checklist.md` line by line against
...
6. `docs/contributing/agent-routines.md` — runbook: how to register each routine
...
4. `.env.schema`: append the three variables with env-spec annotations
   ...
   `docs/contributing/posthog-setup.md`).
...
`tests/redteam` — `@opensesame/redteam`, private; devDependency `promptfoo`
...
**OWNS:** `tests/visual-contract/**`

`tests/visual-contract` — `@opensesame/visual-contract`, private;
...
3. Compare each shot to `.impeccable/screenshots/<name>.png` with pixelmatch,
   ...
   `tests/visual-contract/output/` (gitignored). `--update` mode
...
**OWNS:** `docs/contributing/linear-workflow.md`, `docs/contributing/posthog-setup.md`
