...
| Persistence: `certificate_authorities`, `certificate_issuance_requests`, `issued_certificates` | `crates/storage/migrations/0013_certificate_issuance.sql`, `crates/storage/src/lib.rs` |
...
5. **Org scoping.** All new tables carry `organization_id` with composite
   ...
   `crates/storage/migrations/0013_certificate_issuance.sql` does. Cross-org reads are adversarially tested
...
8. **Contract tests.** Every new route is either documented in `spec/openapi/host-api.yaml` or
...
- **ADR 0066 — Certificate Manager domain model.** Introduces CA hierarchy
  ...
  linkage). Supersedes the "(dev TLS)" positioning in `docs/research/competitors/infisical.md`. Records
...
`docs/validation/certificate-manager.md` evidence doc, `docs/research/competitors/infisical.md`
(rewrite the certificate rows), `docs/reference/standards-matrix.md` (RFC 5280, 8555, 7030, 8894, 6960,
...
### 4.1 Migration DDL — `crates/storage/migrations/0016_certificate_manager.sql`
...
`spec/openapi/host-api.yaml` or the `contract.rs` allowlist.
...
- **Owns:** `docs/adr/0066-*.md`…`0072-*.md`; `docs/validation/certificate-manager.md`; new rows in
  ...
  `docs/reference/standards-matrix.md` RFC rows; rewrite of the certificate rows in
  `docs/research/competitors/infisical.md`.
...
- **Owns:** `crates/storage/migrations/0016_certificate_manager.sql`; the `MIGRATIONS` entry in
...
- **Detail:** a provider-agnostic library — no axum, no storage. Fuzz targets for CSR + PKCS#12
  parsing under `tests/fuzz/cargo/fuzz_targets/` (register in `tests/fuzz/cargo/Cargo.toml`, model
...
- **Owns (exclusively) the shared files:** `apps/gateway/src/routes/mod.rs` (mount every §4.2 route
  ...
  consequential new routes), `spec/openapi/host-api.yaml` + `apps/gateway/src/routes/contract.rs`
...
| **Contract / pact** | `pact` test modules; route↔spec test `apps/gateway/src/routes/contract.rs` | `*pact.test.ts` — `packages/database/tests/pact.test.ts`, `tests/redteam/src/structural.pact.test.ts` |
...
| **Fuzz** | `tests/fuzz/cargo/fuzz_targets/*.rs` + `tests/fuzz/cargo/Cargo.toml` — model `certificate_request.rs`; `pnpm audit:fuzz` | Jazzer.js — `tests/fuzz/jazzer`, `pnpm test:fuzz` |
...
New fuzz targets registered in `tests/fuzz/cargo/Cargo.toml`: CSR parser, PKCS#12 parser, SCEP CMS parser,
...
|---|---|
| §5.1 ADR & docs | `docs/adr/0066..0072-*.md`, `docs/validation/certificate-manager.md`, rows in `docs/security/threat-model.md`, `docs/security/key-hierarchy.md`, `docs/reference/standards-matrix.md`, `docs/research/competitors/infisical.md` |
| §5.2 Persistence | `crates/storage/migrations/0016_certificate_manager.sql`, additions to `crates/storage/src/lib.rs` |
...
| §5.20 Assembler | `routes/mod.rs`, `app_state.rs`, `main.rs`, `spec/openapi/host-api.yaml`, `routes/contract.rs`, `packages/capability-registry/src/index.ts`, `apps/mcp-host/src/tools.ts`, `apps/pages/src/components/AppShell.tsx`, `packages/app-core/src/webmcp/tools.ts` (SECTION_PATHS), `.env.schema`, `AGENTS.md`, root `Cargo.toml` |
