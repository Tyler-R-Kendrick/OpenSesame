...
- Consumers of this crate: sealed-store, client-core (Wasm re-export), fuzz
  target `tests/fuzz/cargo/fuzz_targets/vault_envelope.rs`. Do not change existing
...
- Fuzz targets in `tests/fuzz/cargo/fuzz_targets/` (mirror `vault_envelope.rs`;
  register in `tests/fuzz/cargo/Cargo.toml`).
...
new `crates/storage/migrations/0011_attachment_targets.sql`.
...
2. New `tests/fuzz/cargo/fuzz_targets/attachment_chunk.rs` (+ `tests/fuzz/cargo/Cargo.toml`
