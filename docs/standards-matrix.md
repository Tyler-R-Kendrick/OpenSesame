...
| RFC 9325 TLS BCP | BCP | Product profile, not a conformance claim: TLS 1.3 default, TLS 1.2 only by explicit `*_MIN_VERSION=1.2` with rustls safe defaults, nothing older; resumption, tickets and 0-RTT disabled on `mtls_required` / `trusted_ingress` server profiles (ADR 0130 §10) | `crates/transport-security` |
| NATS auth callout (native, ADR-26) | Ecosystem | `$SYS.REQ.USER.AUTH` request/response with NKey/JWT verification, one-time user key, server context and response binding; Host verifies the user's upstream token itself. `opensesame.callout.>` is an unrelated internal namespace. Client mTLS (`verify: true`) and certificate mapping (`verify_and_map: true`) are distinct server profiles; client connections only (ADR 0130 §8) | `crates/nats-callout`, `apps/gateway/src/routes/nats_callout*.rs`, `ops/nats/` |
...
| RFC 7030 EST | Final | **Absent — unsupported.** No EST server or client exists in the tree; `apps/gateway/src/routes/est_server.rs` (named by ADR 0068 §4 as forthcoming) was never created and is not a prerequisite of mTLS (ADR 0130 §14) | — |
...
| PKCS#11 v2.40 | Final | **Absent — unsupported.** ADR 0071 design; no `cryptoki` dependency and neither `crates/hsm-client` nor `crates/pkcs11-provider` exists. No HSM or KMS signer implements TLS signing; transport identities are software custody only (ADR 0130 §2) | — |
...
([ADR 0130](adr/0130-optional-mtls-and-workload-identity.md)). The ADR 0130
...
`apps/gateway/src/middleware/auth.rs` were not yet in the tree — ADR 0130
