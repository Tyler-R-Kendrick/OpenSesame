...
| --- | --- | --- |
| Every plan × method ends in a person's token | `pnpm --filter @opensesame/pages exec vitest run src/lib/connect-conformance` | 223 paths across 188 services, through the real relay handlers, a schema-strict Connect emulator and a strict OAuth provider emulator; 215 confirmed by the service's verify call. Negative controls fail as a provider would. |
| Every real endpoint is live | `pnpm test:connect-preflight` | 46 OAuth authorization servers, 98 MCP servers, 66 API-key verify endpoints; 9 need the customer's own host. |
