...
-- —— 1. lifecycle_hooks: a2h joins webhook and internal ————————————
--
-- An a2h hook has the same shape as a webhook one: an endpoint (the A2H
-- gateway's base URL) and a sealed whsec_ secret (which signs the gateway's
-- callback to us rather than our request to it). So both CHECK groups widen the
-- same way, and 'internal' keeps its exact inverse shape.
CREATE TABLE IF NOT EXISTS lifecycle_hooks_widened (
  id TEXT PRIMARY KEY,
  organization_id TEXT NOT NULL,
  name TEXT NOT NULL,
  event_types_json TEXT NOT NULL,
  delivery TEXT NOT NULL CHECK (delivery IN ('webhook', 'internal', 'a2h')),
  endpoint_url TEXT,
  responder TEXT,
  ...
  CHECK (
    (delivery IN ('webhook', 'a2h') AND endpoint_url IS NOT NULL AND length(endpoint_url) > 0
      AND responder IS NULL)
    OR
    (delivery = 'internal' AND responder IS NOT NULL AND length(responder) > 0
      AND endpoint_url IS NULL)
  ),
  -- An internal responder still never holds a signing secret: it is called in
...
INSERT OR IGNORE INTO lifecycle_hooks_widened
SELECT id, organization_id, name, event_types_json, delivery, endpoint_url, responder,
       subject_kinds_json, enabled, sealed_secret_key_id, sealed_secret_ciphertext,
       sealed_secret_nonce, sealed_secret_aad_digest, last_delivered_at, last_error,
       version, created_at, updated_at
FROM lifecycle_hooks;

DROP TABLE lifecycle_hooks;
ALTER TABLE lifecycle_hooks_widened RENAME TO lifecycle_hooks;
