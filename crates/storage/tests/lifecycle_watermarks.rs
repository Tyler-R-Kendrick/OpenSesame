...
//!
//! This exists because it did not, twice — and because the first fix only
//! caught half of it. `0017_lifecycle_hooks.sql` enumerated the five subject
//! kinds that existed when the expiry ladder was written. `web_login`
//! (ADR 0081) and `session_grant` (ADR 0079) were added to [`SubjectKind`] in
//! the same window, and `0023` widened the `CHECK` for the first of them only.
//!
...
//!
//! So the fence is not "remember the migration", which is what failed both
//! times. It is this file: adding a variant to [`SubjectKind::ALL`] without
//! widening the `CHECK` fails here, with a message naming the consequence.
...
#[tokio::test]
async fn the_kind_the_first_fix_missed_can_claim_a_rung() {
    // `0023` widened the CHECK for `web_login` and stopped there.
    // `session_grant` arrived in the same window and was still refused, so a
    // shared-session grant approaching its deadline re-published its notice on
    // every scan pass and never recorded that it had fired.
    let db = db().await;
    let now = Utc.with_ymd_and_hms(2026, 8, 31, 0, 0, 0).unwrap();
    assert!(db
        .record_lifecycle_watermark(
            &watermark(SubjectKind::SessionGrant, "alert", "urgent"),
            now
        )
        .await
        .is_ok());
