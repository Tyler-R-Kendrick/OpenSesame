-- The step queue a local driver claims from (ADR 0081, ADR 0079 §4).
--
-- Numbered 0025 because 0024 was already claimed by
-- `0024_session_grant_watermarks.sql`, an independent fix in flight at the
-- time on its own branch. Both have landed and the sequence is contiguous;
-- the number was chosen to avoid a collision, not to leave a gap.
--
...
CREATE TABLE IF NOT EXISTS runner_steps (
  ...
  -- The principal that claimed it. Only the run's owner may (ADR 0081 §8), and
