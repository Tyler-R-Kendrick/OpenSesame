...
//!    bolted on the front.
//!
//! # Failure discipline (inherited from `crates/credential-helpers`)
//!
...
/// The bridge programs, one per cargo feature. `opensesame` runs one when it
/// is invoked under the bridge's name (`opensesame-browserpass-host`,
/// `opensesame-gopass-jsonapi`, `opensesame-keepassxc-bridge`).
pub mod entry {
