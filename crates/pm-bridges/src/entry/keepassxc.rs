...
use crate::conflict::keepassxc_socket_path;
use crate::keepassxc::pairing::BRIDGE_NAME;
use crate::keepassxc::transport::{self, serve_stdio};
use crate::keepassxc::{Session, SessionConfig};
use crate::pairing::PairingWindow;
use crate::store::StoreAccess;
...
/// Run the bridge; `args` are the arguments after the program name.
pub fn main(args: &[String]) {
    let mode = parse_args(args.iter().cloned());
...
Pairing is a separate human action: `opensesame bridge keepassxc pair`.",
        socket = crate::conflict::KEEPASSXC_SOCKET_NAME
    );
}
