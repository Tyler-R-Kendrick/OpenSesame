...
fn is_writeish(actions: &[String]) -> bool {
    actions.iter().any(|action| {
        ...
        WRITEISH_TOKENS
            .iter()
            .any(|token| names_token(&lower, token))
    })
}

/// Mirror the TS boundary: the whole action, or `token` at start/end or beside . : _
fn names_token(lower: &str, token: &str) -> bool {
    lower == token
        || ['.', ':', '_'].iter().any(|sep| {
            lower.starts_with(&format!("{token}{sep}"))
...
/// # Errors
///
/// `CohortGrantee` for a cohort resource; `UnmappedScope` for a missing or other connection.
