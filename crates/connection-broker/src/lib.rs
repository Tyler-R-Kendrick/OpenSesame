...
impl ConnectionBroker {
    pub fn new(db: Db, config: BrokerConfig) -> Self {
    ...
    pub async fn create_connection(
        ...
        let logical_name = match request.logical_name.filter(|n| !n.trim().is_empty()) {
            Some(name) => {
                ...
                if store::logical_name_taken(&self.db, &organization, &name).await? {
        ...
        store::insert_connection(&self.db, &row).await?;
        store::append_event(&self.db, &row.id, EventKind::Created, Some(provider.id)).await?;
    ...
    async fn unused_logical_name(&self, organization: &str, provider_id: &str) -> Result<String> {
        ...
        if !store::logical_name_taken(&self.db, organization, &base).await? {
            return Ok(base);
        }
        for n in 2..1000u32 {
            let candidate = format!("{base}-{n}");
            if !store::logical_name_taken(&self.db, organization, &candidate).await? {
                return Ok(candidate);
            }
        }
    ...
    pub async fn list_connections_for(
        ...
        let rows = store::list_connections(&self.db, &organization_id.to_string()).await?;
    ...
    pub async fn events(
        ...
        store::list_events(&self.db, id).await
    }
    ...
    async fn row_in_org(
        ...
        let row = store::get_connection(&self.db, id)
            .await?
    ...
    pub async fn start_authorization(
        ...
        let scopes = match scopes.filter(|s| !s.is_empty()) {
            Some(s) => {
                store::set_requested_scopes(&self.db, &row.id, &s).await?;
                s
            }
    ...
    pub async fn complete_authorization(
        ...
        let authorization = store::consume_authorization(&self.db, state, Utc::now()).await?;
        let row = store::get_connection(&self.db, &authorization.connection_id)
            .await?
            .ok_or(BrokerError::ConnectionNotFound)?;
        ...
        .await
        {
            ...
            Err(e) => {
                let detail = e.hint();
                store::set_status(&self.db, &row.id, ConnectionStatus::Error, Some(&detail))
                    .await?;
                store::append_event(&self.db, &row.id, EventKind::Error, Some(&detail)).await?;
        ...
        store::set_grant_details(&self.db, &row.id, &tokens.scopes, account_label.as_deref())
            .await?;
        store::set_status(&self.db, &row.id, ConnectionStatus::Active, None).await?;
        store::append_event(&self.db, &row.id, EventKind::Authorized, Some(provider.id)).await?;
    ...
    pub async fn refresh(
        ...
        let credential = store::get_credential(&self.db, &row.id)
            .await?
        ...
        match token::refresh(&self.http, provider, &self.config, &tokens).await {
            Ok(next) => {
                ...
                store::set_grant_details(&self.db, &row.id, &next.scopes, None).await?;
                store::set_status(&self.db, &row.id, ConnectionStatus::Active, None).await?;
                store::append_event(&self.db, &row.id, EventKind::Refreshed, None).await?;
            ...
            Err(e) => {
                ...
                .await?;
                store::append_event(&self.db, &row.id, EventKind::RefreshFailed, Some(&detail))
                    .await?;
    ...
    async fn credential_moved_on(
        ...
        let Ok(Some(current)) = store::get_credential(&self.db, &row.id).await else {
            return false;
        };
    ...
    pub async fn revoke(
        ...
        store::delete_credential(&self.db, &row.id).await?;
        store::set_status(&self.db, &row.id, ConnectionStatus::Revoked, None).await?;
        store::append_event(&self.db, &row.id, EventKind::Revoked, None).await?;
    ...
    async fn revoke_upstream(
        ...
        let Ok(Some(credential)) = store::get_credential(&self.db, &row.id).await else {
            return ProviderRevocation::Unsupported;
        };
    ...
    pub async fn unbind(
        ...
        let row = self.row_in_org(organization_id, id).await?;
        store::delete_binding(&self.db, &row.id, binding_id).await?;
        store::append_event(&self.db, &row.id, EventKind::Unbound, None).await?;
        self.view(row).await
    }
    ...
    async fn store_tokens(
        ...
        let previous = store::get_credential(&self.db, &row.id).await?;
    ...
    async fn view(&self, row: ConnectionRow) -> Result<ConnectionView> {
        let bindings = store::list_bindings(&self.db, &row.id).await?;
        let credential = store::get_credential(&self.db, &row.id).await?;
