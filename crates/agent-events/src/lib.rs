...
impl AgentEvent {
    ...
    ///
    /// The delivery ledger is the durable queue (ADR 0039's saga shape), and it
    /// stores the [`SecurityNotice`] — whose `payload` is exactly what
    /// [`Self::payload`] built. So a notifier that survives a restart works
    /// from the persisted bytes rather than from an in-memory event, which
    /// makes the payload a two-way contract: whatever a subscriber is given
    /// must be enough to rebuild the fact. The round-trip test is what keeps a
    /// field from being dropped from the wire without anyone noticing.
    ///
...
mod tests {
    ...
    #[test]
    fn a_notice_carries_the_payload_the_ledger_replays_from() {
        // The stored ledger row is the SecurityNotice, so the round trip that
        // matters in production goes through its `payload` field, not through
        // `payload()` directly.
        let deadline = now() + chrono::Duration::seconds(300);
        let original =
            AgentEvent::waiting(run(), AgentPhase::Blocked, now(), deadline, Some("stuck"))
                .unwrap();
        let stored = serde_json::to_string(&original.notice()).unwrap();
        let replayed: SecurityNotice = serde_json::from_str(&stored).unwrap();
        assert_eq!(
            AgentEvent::from_payload(&replayed.payload).as_ref(),
            Some(&original),
        );
    }
