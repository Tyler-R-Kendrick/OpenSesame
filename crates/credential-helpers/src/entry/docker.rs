...
use crate::{docker_protocol, fail, required_env, DaemonClient, HelperError};
use std::io::Read;
...
fn run(verb: Option<&str>) -> Result<(), HelperError> {
    match verb {
        Some("get") => get(),
        Some("list") => {
...
/// Answer one helper invocation; `args` are the arguments after the
/// program name.
pub fn main(args: &[String]) {
    if let Err(error) = run(args.first().map(String::as_str)) {
        fail(error);
    }
}
