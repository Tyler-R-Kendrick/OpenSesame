...
async fn config_mode_callout_end_to_end() {
    ...
    let (good, good_events) = stack.app_client(GOOD_TOKEN).await.expect("good token admitted");
    ...
    good.publish("$SYS.REQ.USER.AUTH", "forged".into()).await.unwrap();
    good.publish("opensesame.events.system.x", "x".into()).await.unwrap();
    good.flush().await.unwrap();
    let violations = wait_for_permission_violations(&good_events, 2).await;
    assert!(violations >= 2, "expected subscribe+publish violations, saw {violations}");
    assert!(
        tokio::time::timeout(Duration::from_millis(300), sys_sub.next()).await.is_err(),
    ...
    assert_eq!(a.user_jwt().map(user_permissions), b.user_jwt().map(user_permissions));
    assert_eq!(stack.host.digests()[2], first_digest);
    assert_eq!(stack.host.digests()[3], first_digest);
    assert_eq!(stack.host.decisions_for(&first_digest), 1, "one immutable decision per digest");
    ...
    //    fresh decision.
    let (short, short_events) = stack.app_client(SHORT_TOKEN).await.expect("short token admitted");
    let calls_before = stack.host.calls();
    let disconnected = wait_for_disconnect(&short_events, Duration::from_secs(12)).await;
    assert!(disconnected, "server must disconnect the client at user JWT expiry");
    tokio::time::sleep(Duration::from_millis(500)).await;
    assert!(stack.host.calls() > calls_before, "reconnect triggers a new callout");
    drop(short);
    drop(good);
    stack.stop().await;
}
...
fn user_permissions(jwt: &str) -> (Vec<String>, Vec<String>, i64) {
    ...
    (u.publish_allow().to_vec(), u.subscribe_allow().to_vec(), u.exp)
}
...
async fn bridge_with_the_wrong_client_certificate_is_refused_by_the_host() {
    ...
    assert_eq!(resp.error(), Some("host_error"), "Host refused the worker certificate; bridge denied");
