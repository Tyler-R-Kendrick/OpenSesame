...
#
#   1. preflight: every crate named in scripts/mtls-required-packages.txt must
...
#   4. scripts/mtls-static-imports.mjs (AT-STATIC-IMPORTS).
#
...
#
# Usage: bash scripts/mtls-test.sh [--skip-build]   (--skip-build reuses apps/pages/dist)
set -euo pipefail
...
REQ_FILE="${ROOT}/scripts/mtls-required-packages.txt"
...
if [[ ${#missing_required[@]} -gt 0 ]]; then
  step --id preflight-packages --claim OPS-RUNNERS --scenarios AT-EVIDENCE-NORUN --required true \
    --expected "every crate in scripts/mtls-required-packages.txt exists in cargo metadata" \
...
ts_step pages-transport false apps/pages/src/lib/transport-status.ts pnpm --filter @opensesame/pages exec vitest run src/lib/transport src/sections/settings/transport
...
step --id static-imports --claim AT-STATIC-IMPORTS --scenarios AT-STATIC-IMPORTS --runner marker --required true --timeout 1500 \
  --profile static --target "apps/pages/dist" --paths scripts/mtls-static-imports.mjs \
  --expected "no native TLS/fs/net/process/SPIFFE-socket/attestation code in the shipped Pages bundle or its import graph" -- \
  node scripts/mtls-static-imports.mjs "${static_args[@]}"
