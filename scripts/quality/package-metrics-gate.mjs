 ...
 * scripts/quality/quality-gate.mjs ratchets structural debt: the recorded set may
 * shrink, never grow.
 *
 ...
 *
 * Usage:
 *   node scripts/quality/package-metrics-gate.mjs            # check
 *   node scripts/quality/package-metrics-gate.mjs --update   # re-record the baseline
 *   node scripts/quality/package-metrics-gate.mjs --report   # full I/A/D table
 */
...
if (update) {
  writeFileSync(
    ...
    `${JSON.stringify(
      {
        $comment: [
          "Accepted component-coupling debt -- see scripts/quality/package-metrics-gate.mjs.",
