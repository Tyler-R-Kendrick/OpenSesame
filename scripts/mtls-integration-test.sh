...
#
# Fetches the pinned fixtures (scripts/mtls-fixtures.sh), proves each binary
...
# scripts/mtls-required-packages.txt that is missing is FAILED. No verification
...
#
# Usage: bash scripts/mtls-integration-test.sh
set -euo pipefail
...
FIXTURES="${ROOT}/scripts/mtls-fixtures.sh"
...
step --id fixtures-fetch --claim OPS-REPRODUCE --scenarios AT-EVIDENCE-NORUN --required true --timeout 900 \
  --profile fixtures --target "${OPENSESAME_MTLS_FIXTURE_DIR}" --paths scripts/mtls-fixtures.sh \
...
step --id it-browser --claim AT-BROWSER-UX --scenarios AT-BROWSER-EXTERNAL,AT-BROWSER-UX,AT-STATIC-EMPTY --runner marker --required true --timeout $((STEP_TIMEOUT + 60)) \
  --profile "browser (chromium)" --target "${PLAYWRIGHT_CHROMIUM}" --paths scripts/mtls-browser-test.mjs \
  --expected "browser scenarios that exist run; the rest are not_executed, never passed" -- \
  "${BOUNDED[@]}" node scripts/mtls-browser-test.mjs
