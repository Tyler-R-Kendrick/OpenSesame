...
| `bundle-budgets.json` | `scripts/quality/bundle-budget-gate.mjs` (`pnpm quality:bundle`) | KiB ceilings for `total`, `javascript`, `javascriptGzip`, `css` and `largestAsset` of the built `apps/pages`, `apps/pwa` and `apps/console`, plus the hardened capability-profile builds under `apps/pages/dist-profiles/`. | Not auto-recorded. A bundle may grow when a feature lands; raising a number is a hand-written line with a dated reason in the entry's notes. The profile budgets are ceilings to fall from, never raised to pass a build. |
