...
tester.run("anti-slop/no-known-value-widening", noKnownValueWideningRule, {
	valid: [
		...
		"let values: unknown[]; values = [];",
		"function consume(values: unknown[]) {} consume([]);",
		"declare const external: (value: unknown) => void; external({ known: true });",
		"declare function external(value: unknown): void; external({ known: true });",
		"interface Known { known: boolean } const inferred = (value: Known) => {}; inferred({ known: true });",
		"declare const service: { consume(value: unknown): void }; service.consume({ known: true });",
	],
	invalid: [
		...
		{ code: "const value: object = [];", errors: [error] },
		{ code: "const values: unknown[] = [1];", errors: [error] },
		{ code: "let values: unknown[] = []; values = [1];", errors: [error] },
		{
			code: `${prelude} function consume(value: Record<string, Command>) {} consume({ start: startCommand });`,
			errors: [error],
		},
		{
			code: `${prelude} const consume: (value: Record<string, Command>) => void = () => {}; consume({ start: startCommand });`,
			errors: [error],
		},
		{
			code: `${prelude} ((value: Record<string, Command>) => {})({ start: startCommand });`,
			errors: [error],
		},
		{
			code: `${prelude} function consume(value: Record<string, Command> = { start: startCommand }) {}`,
			errors: [error],
		},
		{
			code: `${prelude} class Registry { commands: Record<string, Command> = {}; set() { this.commands = { start: startCommand }; } }`,
			errors: [error],
		},
		{
			code: `${prelude} async function create(): Promise<Record<string, Command>> { return { start: startCommand }; }`,
			errors: [error],
		},
		{
			code: `${prelude} const create = async (): Promise<Record<string, Command>> => ({ start: startCommand });`,
			errors: [error],
		},
	],
});
