...
export interface ImportedFiniteDomainFixture {
	readonly id: string;
}

export interface ImportedMergedEmptyFixture {}
export interface ImportedMergedEmptyFixture {}

export type ImportedSafeDictionaryFixture<T = ImportedFiniteDomainFixture> = Record<
	string,
	T
>;
export type ImportedUnsafeDictionaryFixture<T = unknown> = Record<string, T>;
export type ImportedUnsafeUnionFixture<T = ImportedFiniteDomainFixture> =
	| T
	| ImportedUnsafeDictionaryFixture;

const tester = new RuleTester({ languageOptions: { parserOptions: { lang: "ts" } } });

const error = { messageId: "unsafeDictionary" };
const fixtureFilename = fileURLToPath(new URL("./no-unsafe-dictionary-type.ts", import.meta.url));
const fixtureImport = "./no-unsafe-dictionary-type.test.ts";
...
tester.run("anti-slop/no-unsafe-dictionary-type", noUnsafeDictionaryTypeRule, {
	valid: [
		...
		"type Node = string | readonly Node[] | { readonly [key: string]: Node }; type A = Record<string, Node>;",
		"type Left = Right; type Right = Left; type A = Record<string, Left>;",
		{
			code: `import type { ImportedFiniteDomainFixture } from "${fixtureImport}"; type A = Record<string, ImportedFiniteDomainFixture>;`,
			filename: fixtureFilename,
		},
		{
			code: `import type { ImportedSafeDictionaryFixture as Safe } from "${fixtureImport}"; type A = Safe;`,
			filename: fixtureFilename,
		},
		{
			code: `import type { ImportedFiniteDomainFixture, ImportedUnsafeDictionaryFixture as Dictionary } from "${fixtureImport}"; type A = Dictionary<ImportedFiniteDomainFixture>;`,
			filename: fixtureFilename,
		},
	],
	invalid: [
		...
		{
			code: "type Escape<T> = T | unknown; type Index<T> = Record<string, Escape<T>>; type A = Index<Command>;",
			errors: 2,
		},
		{
			code: "type Unsafe<T> = Record<string, T>; type MaybeUnsafe<T> = Command | Unsafe<T>; type A = MaybeUnsafe<unknown>;",
			errors: 1,
		},
		{
			code: "type Recursive<T> = T | readonly Recursive<T>[]; type A = Record<string, Recursive<unknown>>;",
			errors: 1,
		},
		{
			code: `import type { ImportedUnsafeDictionaryFixture as Unsafe } from "${fixtureImport}"; type A = Unsafe;`,
			filename: fixtureFilename,
			errors: 1,
		},
		{
			code: `import type { ImportedUnsafeDictionaryFixture as Unsafe } from "${fixtureImport}"; type A = Readonly<Unsafe>;`,
			filename: fixtureFilename,
			errors: 1,
		},
		{
			code: `import type { ImportedUnsafeUnionFixture as Unsafe } from "${fixtureImport}"; type A = Unsafe;`,
			filename: fixtureFilename,
			errors: 1,
		},
		{
			code: `import type { ImportedMergedEmptyFixture } from "${fixtureImport}"; type A = Record<string, ImportedMergedEmptyFixture>;`,
			filename: fixtureFilename,
			errors: [error],
		},
	],
