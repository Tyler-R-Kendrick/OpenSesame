...
| `examples/*` | Example relying parties (`rp-alpha`, `rp-beta`, `static-rp`; the SIOP one is `apps/example-siop-rp`, the Vercel project's root), agents (`agent`, `static-agent`) and a headless device-login client (`headless`) |
