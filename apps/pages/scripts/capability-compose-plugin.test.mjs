    ...
    build: {
      rollupOptions: {
        input: {
          ...
          msalRedirect: join(appRoot, "auth/redirect.html"),
        },
      },
    },
  };
...
describe("virtual modules", () => {
  test("selective: every owned document module is in the table; both HTML entries stay", async () => {
    const { userConfig, table, distribution } = await compose({
      mode: "selective",
    });
    for (const id of [
      "connectors.external/runtime",
      ...
      assert.match(
        table,
        new RegExp(
          `"${id.replace(".", "\\.")}": \\(\\) => import\\("${appRoot}/src/modules/${id.split("/")[0].replace(".", "\\.")}/runtime\\.ts"\\)`,
        ),
      );
    }
    assert.doesNotMatch(
  ...
  test("hardened: only the selected closure (+core) is distributed; excluded HTML entry is dropped", async () => {
    ...
    assert.equal(
      chunkFor(`${appRoot}/src/sections/connections/List.tsx`),
      "cap-connectors.external",
    );
    assert.equal(chunkFor(`${appRoot}/src/lib/kv.ts`), undefined);
    assert.equal(
      chunkFor(`${appRoot}/src/sections/connections/a.css`),
      undefined,
...
describe("invalid profiles throw (BUILD-05)", () => {
  const rejects = (options, pattern) =>
    assert.rejects(compose(options), pattern);
  test("parse error", () =>
    rejects({ profilePath: profileFile("broken", "{ not json") }, /not JSON/));
  test("unknown capability", () =>
    rejects(
      {
        profilePath: profileFile("unknown", {
  ...
  test("contradictory policy: a capability both required and prohibited", () =>
    rejects(
      {
        profilePath: profileFile("contra", {
  ...
  test("contradictory policy: a core capability in a policy set", () =>
    rejects(
      {
        profilePath: profileFile("coreset", {
  ...
  test("a selected root the policy prohibits is dropped (runtime PROHIBITED_BY_INSTANCE), not a build error", async () => {
    const profilePath = profileFile("prohibited", {
  ...
  test("a required root not yet accepted is still distributed (runtime REQUIRED_NOT_ACCEPTED)", async () => {
    const profilePath = profileFile("unaccepted", {
  ...
  test("absent module entry file", async () => {
    ...
    try {
      ...
      writeFileSync(
        join(appRoot, "src/modules/sharing.drops/runtime.ts"),
        "export const capabilityRuntime = {};\n",
      );
    }
  });
