...
import { agentRunSeams } from "../../lib/agent-runs.js";
import { RunViewer } from "./RunViewer.js";

// The viewer takes its reads through `agentRunSeams`, so a test supplies
// stand-ins for that object rather than rewriting the module underneath the
// component. Restored wholesale afterwards, so one test cannot leak into the
// next.
const realSeams = { ...agentRunSeams };
const api = {
  readRun: vi.fn(),
  readLog: vi.fn(),
...
describe("RunViewer", () => {
  beforeEach(() => {
    ...
    Object.assign(agentRunSeams, api);
  ...
  afterEach(() => {
    cleanup();
    Object.assign(agentRunSeams, realSeams);
    vi.useRealTimers();
  });
  ...
  it("offers to ask for the page while the agent drives, not to take it", async () => {
    ...
    // it is not possible: it parks first (ADR 0081 §5).
