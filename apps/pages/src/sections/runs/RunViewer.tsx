...
const LANE_LABEL: Record<LogEntry["lane"], string> = {
  action: "did",
  thought: "thinking",
  frame: "screen",
};
 ...
 * our own chrome, beside real controls (ADR 0078 §4). Three things keep that
...
export function RunViewer({
  ...
  async function guard(work: () => Promise<unknown>, done: string) {
    setBusy(true);
    setNotice(null);
    try {
      await work();
      ...
      <div className="run__do">
        {canRequestHandoff(run) && (
          <button
            ...
            onClick={() =>
              void guard(async () => {
                const outcome = await requestHandoff(run.id);
                setNotice(
                  outcome === "queued"
        ...
        {canTakeControl(run) && (
          <button
            ...
            disabled={busy}
            onClick={() =>
              void guard(() => takeControl(run.id), "You have the page.")
            }
          >
            Take the page
          </button>
