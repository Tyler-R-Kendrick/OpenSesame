...
import { FIXTURE_CATALOG, FIXTURE_MANAGED_POLICY } from "./composition-fixture.js";
import { double, moduleTableSpy, resetDouble } from "./test-support.js";
...
vi.mock("../../lib/capabilities/module-table.js", async () => {
  const { moduleTableSpy } = await import("./test-support.js");
  return { MODULE_TABLE: new Proxy({}, { get: (_t, key) => moduleTableSpy(key) }) };
});
...
function pick(id: string) {
  ...
  if (!(button instanceof HTMLButtonElement)) throw new Error(`no pick on ${id}`);
  return button;
}
...
describe("CONSENT-01 — the cards import nothing and connect to nothing", () => {
  it("renders every card, opens every detail sheet and toggles roots without a single import or request", () => {
    ...
    for (const descriptor of FIXTURE_CATALOG.capabilities) {
      ...
      fireEvent.click(screen.getByRole("button", { name: `Details of ${descriptor.title}` }));
  ...
  it("states every fact from the descriptor, including egress and permissions", () => {
    ...
    fireEvent.click(road("Customize this installation"));
    fireEvent.click(screen.getByTestId("purpose-card-custom"));
    fireEvent.click(screen.getByRole("button", { name: "Details of Push notifications" }));
...
describe("purpose cards are presets, previewed until Apply", () => {
  it("renders one card per preset and pre-ticks the preset's defaults as a draft", () => {
    render(<CapabilitySetup />);
    fireEvent.click(road("Customize this installation"));
    for (const id of ["personal", "family", "homelab", "organization", "custom"]) {
      expect(screen.getByTestId(`purpose-card-${id}`)).toBeTruthy();
    }
    fireEvent.click(screen.getByTestId("purpose-card-homelab"));
    expect(pick("connectors.external").getAttribute("aria-pressed")).toBe("true");
    expect(pick("backup.git-remote").getAttribute("aria-pressed")).toBe("true");
    expect(card("connectors.external").textContent).toContain("not yet applied");
    expect(double.commits).toHaveLength(0);
    expect(double.getSnapshot().plan?.approvedCapabilities).not.toContain("connectors.external");
  });
});
...
describe("CONSENT-03 — apply commits exactly the reviewed roots", () => {
  it("commits the draft's roots with a receipt bound to the reviewed plan, then reports the outcome", async () => {
    ...
    await waitFor(() => expect(screen.getByTestId("capability-outcome")).toBeTruthy());
    expect(double.commits).toHaveLength(1);
    const [{ draft, receipt }] = double.commits;
    expect(draft.selectedOptional).toEqual(["agents.webmcp", "backup.git-remote", "connectors.external"]);
    ...
    expect(screen.getByTestId("capability-outcome").textContent).toContain("saved on this device");
    expect(double.getSnapshot().plan?.approvedCapabilities).toContain("agents.webmcp");
  });
  ...
  it("reports session-only when the store cannot write durably", async () => {
    ...
    fireEvent.click(screen.getByTestId("capability-apply"));
    await waitFor(() => expect(screen.getByTestId("capability-outcome").textContent).toContain("this session only"));
...
describe("dependency conflicts explain before/after and offer the alternative", () => {
  it("shows the prohibited dependency and the core alternative, never changing the prohibition", () => {
    ...
    expect(card("connectors.external").textContent).toContain("prohibited by operator");
    fireEvent.click(pick("backup.git-remote"));
    fireEvent.click(screen.getByRole("button", { name: "Save on this device" }));
    const review = screen.getByTestId("capability-review");
    expect(review.textContent).toContain("which this instance prohibits");
    expect(screen.getByRole("button", { name: /Encrypted file backup/ })).toBeTruthy();
    expect(screen.getByTestId("capability-apply").hasAttribute("disabled")).toBe(true);
    expect(double.getSnapshot().policy?.capabilities.prohibited).toContain("connectors.external");
    fireEvent.click(screen.getByRole("button", { name: /Encrypted file backup/ }));
    expect(screen.getByTestId("capability-review").textContent).not.toContain("prohibits");
    expect(double.commits).toHaveLength(0);
  });
});
...
describe("CONSENT-10 — export is a file, publish is absent without its capability", () => {
  it("exports the instance configuration and reads it back equal; offers no publish control", async () => {
    ...
    expect(screen.queryByRole("button", { name: "Publish deployment configuration" })).toBeNull();
    fireEvent.click(screen.getByRole("button", { name: "Export instance configuration" }));
    expect(saved).toHaveLength(1);
    const { reimportMatches } = await import("../../lib/configuration/capabilities-export.js");
    expect(reimportMatches(saved[0]?.[1] ?? "", double.getSnapshot())).toBe(true);
    expect(fetchSpy).not.toHaveBeenCalled();
  });
  ...
  it("offers publish only once a publication capability is approved", async () => {
    ...
    fireEvent.click(screen.getByTestId("capability-apply"));
    await waitFor(() => expect(screen.getByTestId("capability-outcome")).toBeTruthy());
    expect(screen.getByRole("button", { name: "Publish deployment configuration" })).toBeTruthy();
  });
});
