...
describe("needsAttention", () => {
  ...
  it("leaves a binding's own attention to the binding", () => {
    // The key vault waiting on nothing still never lands in this count:
    // only a configured endpoint that stopped answering does.
    const list: ConnectorStatus[] = [
  ...
  it("counts a configured Identity endpoint that stopped answering", () => {
    const list: ConnectorStatus[] = [
      {
        id: "identity",
        name: "Identity",
