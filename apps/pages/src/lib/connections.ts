...
export async function createConnection(input: {
  ...
  const body = await call<Record<string, unknown>>("/connections", {
    method: "POST",
    body: JSON.stringify({
...
export async function removeConnection(id: string): Promise<void> {
  await call(`/connections/${encodeURIComponent(id)}`, { method: "DELETE" });
}
...
export function testProvider(id: string): Promise<{
  ...
  return call(`/providers/${encodeURIComponent(id)}/test`, { method: "POST" });
}
