...
describe("policy validity and the offline envelope (S03, TRUST-09/10)", () => {
  it("judges the window by the supplied clock only", () => {
    ...
    expect(policyValidity(WINDOW, "2026-08-31T23:59:59.000Z")).toBe(
      "not-yet-valid",
    );
    ...
    expect(
      policyValidity({ expires: "never" }, "2026-09-15T00:00:00.000Z"),
    ).toBe("malformed-time");
  });

  it("an expired policy keeps running and keeps governing but accepts nothing new", () => {
    expect(offlineAllowance("valid")).toEqual({
      keepRunning: true,
      acceptNew: true,
      remainsCeiling: true,
    });
    expect(offlineAllowance("expired")).toEqual({
      keepRunning: true,
      acceptNew: false,
      remainsCeiling: true,
    });
...
describe("provenance (S03)", () => {
  it("classifies sources without inferring from content", () => {
    ...
    expect(classifyPolicySource({ kind: "runtime-config" })).toBe(
      "same-origin-deployment",
    );
    expect(
      classifyPolicySource({ kind: "signed-import", verified: true }),
    ).toBe("signed-import");
    expect(
      classifyPolicySource({ kind: "signed-import", verified: false }),
  ...
  it("labels a same-origin deployment honestly and never calls it verified", () => {
    ...
    expect(provenanceCopy("invitation-unverified").claim).toMatch(
      /fingerprint/i,
    );
