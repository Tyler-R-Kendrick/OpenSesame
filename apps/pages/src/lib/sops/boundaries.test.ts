...
describe("SB-034 a sealed identity needs a real, unlocked, non-guest session", () => {
  ...
  it("reads identities from the addressed tomb and never scans another", () => {
    const source = readFileSync(
      join(pagesSrc, "sections/settings/sops/identities.ts"),
      "utf8",
    );
...
describe("SB-069 a vault import lands whole or not at all", () => {
  it("the import writes once and the store's batch write is one change", () => {
    const hook = readFileSync(
      join(pagesSrc, "sections/settings/sops/useVaultSecrets.ts"),
      "utf8",
    );
    // A `for (… of imported.items) await saveItem(…)` is exactly the
    // partial-import bug this case exists to catch.
    expect(hook).not.toMatch(/for \([^)]*imported\.items\)/u);
    expect(hook).toMatch(/await saveItems\(imported\.items\)/u);

    const store = readFileSync(join(pagesSrc, "lib/vault/store.ts"), "utf8");
