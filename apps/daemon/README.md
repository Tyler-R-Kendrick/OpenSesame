...
The local host agent on the Host/authority plane, binary `opensesame-daemon`,
listening on `127.0.0.1:18790` and optionally on a Unix socket. It hands out
short-lived session capabilities to devcontainers, WSL, the toolbar and the
...
- **Used by:** [`apps/cli`](../cli) (`opensesame daemon install|start|status|logs|stop`),
  [`apps/toolbar`](../toolbar), [`apps/credential-helpers`](../credential-helpers)
  (`POST /v1/mint` over the socket), [`apps/mcp-host`](../mcp-host) and
  [`apps/browser-extension`](../browser-extension) (health probe).
