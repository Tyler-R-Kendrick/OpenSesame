...
/// A2H uses one shared secret in both directions: we send it in `callback.secret`
/// and the gateway signs its reply with it.
///
/// # Errors
///
/// Returns the reason the secret could not be opened, without the ciphertext.
pub fn open_hook_secret(state: &AppState, hook: &StoredLifecycleHook) -> anyhow::Result<String> {
    open_secret(state, hook).map_err(|failure| anyhow::anyhow!(failure.detail().to_string()))
}
