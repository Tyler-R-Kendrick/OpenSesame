...
pub async fn callback(State(st): State<AppState>, headers: HeaderMap, body: String) -> Response {
    ...
    let Ok(Some(delivery)) = st.db.get_lifecycle_delivery(&response.responds_to).await else {
        return refused();
    };
    let Ok(Some(hook)) = st
        .db
    ...
    let Ok(secret) = crate::lifecycle::delivery::open_hook_secret(&st, &hook) else {
        return refused();
    };
    ...
    if let Err(error) = st
        .db
        .mark_lifecycle_delivered(&delivery.id, Utc::now())
        .await
    {
...
/// actually asked.
fn delivery_intent(
    delivery: &opensesame_storage::StoredLifecycleDelivery,
