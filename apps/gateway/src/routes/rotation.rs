...
mod web_login_policy_tests {
    ...
    use crate::test_principals::{P11, P12};
    ...
    fn a_session_owns_its_own_policies() {
        ...
        assert_eq!(
            policy_owner(&session(P11), &target, None).ok(),
            Some(Some(P11.into()))
        );
        assert_eq!(
            policy_owner(&session(P11), &target, Some(P11)).ok(),
            Some(Some(P11.into()))
        );
    }
    ...
    fn a_session_cannot_enrol_somebody_elses_account() {
        ...
        assert!(policy_owner(&session(P11), &target, Some(P12)).is_err());
    }
    ...
    fn an_operator_must_name_an_owner_for_a_web_login() {
        ...
        assert_eq!(
            policy_owner(&Caller::Operator, &web, Some(P11)).ok(),
            Some(Some(P11.into()))
        );
    }
