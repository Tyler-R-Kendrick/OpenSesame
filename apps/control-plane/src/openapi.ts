...
export function buildOpenApiDocument(config: ControlPlaneConfig) {
  return {
    ...
    paths: {
      ...
      "/agent/identity": {
        post: {
          summary: "Register an agent identity (auth.md AgentAuth)",
          requestBody: {
            required: true,
            content: {
              "application/json": {
                  ...
                  oneOf: [
                    {
                      type: "object",
                      required: ["type"],
                      properties: { type: { const: "anonymous" } },
                    },
                    {
                      type: "object",
                      required: ["type", "login_hint"],
                      properties: {
                        type: { const: "service_auth" },
          ...
          responses: {
            ...
            "400": { description: "Protocol error" },
            "429": { description: "Rate fence" },
          },
        },
      },
      "/agent/identity/claim": {
        post: {
          summary: "Start or restart an AgentAuth claim ceremony",
          responses: {
            "200": { description: "Claim attempt minted" },
            "400": { description: "Invalid claim token" },
            "410": { description: "Claim expired" },
          },
        },
      },
      "/oauth2/token": {
        post: {
          summary:
            "JWT-bearer assertion exchange or AgentAuth claim poll (RFC 7523 / auth.md)",
          responses: {
      ...
      "/oauth2/revoke": {
        post: {
          summary: "RFC 7009 revocation for AgentAuth access tokens",
          responses: {
            "200": { description: "Revoked or already unknown" },
            "400": { description: "Malformed body" },
          },
        },
      },
      "/v1/agent-resources/demo": {
        get: {
          summary: "Sample AgentAuth protected resource",
          responses: {
            "200": { description: "Authorized" },
            "401": { description: "Missing/invalid agent access token" },
      ...
      "/v1/authorization-requests/inbox-ref": {
        get: {
          summary:
            "The caller's own inbox handle, to share with whoever may ask",
          description:
      ...
      "/v1/interactions": {
        ...
        post: {
          ...
          responses: {
            "201": {
              description:
                "Interaction created; the reference is safe to print",
              content: {
                "application/json": {
      ...
      "/v1/interactions/{ref}/consume": {
        post: {
          ...
          description:
            "The requester's route. A compare-and-set on the row version serializes two executors racing to spend one approval; the loser gets 409. Spending an interaction that carries no approval answers 401 approval_required — the caller is authenticated and is the right caller, and what is missing is the ceremony.",
          security: [{ bearerAuth: [] }, { provisionalCookie: [] }],
          parameters: [
            {
              name: "ref",
        ...
        InteractionError: {
          ...
          properties: {
            ...
            fields: {
              type: "array",
              description:
                "Paths that failed validation. Never the values at them.",
              items: { type: "string" },
            },
            detail: { type: "string" },
          },
