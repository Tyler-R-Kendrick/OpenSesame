...
// Booting PGlite and applying every migration is setup, not the behaviour
// under test: under a loaded CI runner it alone took most of the 15s test
// budget, so it runs in a hook with the same 60s budget as the other PGlite
// suites (oauth-client-sector-key-db, openid4vci-issuance).
beforeAll(async () => {
  client = new PGlite();
...
it("persists registered actors and their claims across app instances, with a single quota winner", async () => {
  ...
  const ownerResponse = await first.app.request("/v1/principals/provisional", {
    method: "POST",
  });
  ...
  expect(await getAgentUsage(restarted.ctx.stores, owner.principalId)).toBe(2);
