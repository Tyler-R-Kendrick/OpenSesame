...
Local-IPC bridge binaries on the Host/authority plane that let foreign
...
reachable from the agent plane. Every serving surface is off by default.

## Where it fits

- **Used by:** [`apps/cli`](../cli) (`opensesame bridge …` installs
...
- Failure discipline follows [`apps/credential-helpers`](../credential-helpers):
  non-zero exit, only a well-formed protocol error on stdout, only a failure
  class on stderr.

## Surface

| Cargo feature | Binary | Protocol |
|---|---|---|
...
cargo +1.88.0 build -p opensesame-pm-bridges --features keepassxc
```

The golden and handshake suites in `tests/` only compile with their features
on. `keepassxc_handshake.rs` runs real `crypto_box` operations over a
temporary Unix socket; protocol replies are pinned as `insta` snapshots in
`tests/snapshots/`.
