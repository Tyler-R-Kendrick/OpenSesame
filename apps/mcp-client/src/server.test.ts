...
vi.mock("@opensesame/api-client", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("@opensesame/api-client")>();
...
describe("model payload helpers", () => {
  ...
  it("modelError extracts Error messages and stringifies non-Errors", () => {
    ...
    expect(payload(fromError as ToolResult)).toEqual({
      error: "op_failed",
      message: "boom",
    });

    const fromString = modelError("op_failed", "nope");
    expect(payload(fromString as ToolResult)).toEqual({
      error: "op_failed",
      message: "nope",
    });
  });
  ...
  it("modelError drops credential-looking messages from model output", () => {
    ...
    expect(payload(result as ToolResult)).toEqual({ error: "op_failed" });
  });
});
