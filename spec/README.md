...
| [`conformance/vault-vectors.json`](conformance/vault-vectors.json) | **The golden Pages vault vectors**: a sealed export, a personal and a project offline backup and a legacy unbound export, with the password (and its NFKC form), a PIN and a passkey PRF output. Each opens to a recorded tomb, binding, revision and item list ([vault format v1](../docs/architecture/vault-format-v1.md)). Never regenerated to make a test pass. | `packages/vault-core` (`openVaultFile`), `packages/app-core` (the store and the bare-isolate proof), the client CLI (`opensesame-id vault`), `crates/human-vault` (`pages_vault`, the native `opensesame vault`) |
