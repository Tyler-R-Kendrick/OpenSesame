# Agent setup for `opensesame`

This is a deepsec scanning workspace. Project `opensesame` was just registered
(target: `..`). Setup is incomplete — `data/opensesame/INFO.md`
...
2. **Fill in `data/opensesame/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id opensesame
