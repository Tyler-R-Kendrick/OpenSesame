...
import {
  INTERACTION_APPROVAL_POLICY,
  interactionApprovalPolicyDigest,
  spendInteractionActivation,
} from "./interaction-activation-spend.js";
import { attachInteractionTotpRoute } from "./interaction-activation-totp.js";
...
async function beginActivation(
  ...
  const settled = refuseIfSettled(c, deps, row);
  ...
  const method = parsed.data.method ?? "webauthn";
  if (method === "totp" && interactionRequiresPhishingResistance(row.kind)) {
    return deps.fail(c, "invalid_request");
  }
  if (
  ...
  const policyDigest = interactionApprovalPolicyDigest(row.kind);
  ...
  const totpNonce = randomBytes(32).toString("base64url");
  const issued =
    method === "totp"
      ? { challenge: totpNonce, options: {} }
      : await issueTransactionChallenge(ctx.passkeyChallenges, rp, {
          principalId,
          transactionDigest,
  ...
  const activation: ApprovalActivation = {
    ...
    policyDigest,
    channelKind: "in_app",
    challengeDigest: challengeDigest(issued.challenge),
    state: "pending",
    createdAt: now,
    expiresAt: new Date(now.getTime() + ttlSeconds * 1000),
    version: 1,
    ...(method === "totp" ? { method: "totp" } : undefined),
  };
...
export function attachInteractionActivationRoutes(
  ...
  routes.post("/:ref/activation", requirePrincipal(), (c) =>
    beginActivation(c, deps),
  );
  routes.post("/:ref/activation/complete", requirePrincipal(), (c) =>
    completeActivation(c, deps),
  );
  attachInteractionTotpRoute(routes, deps);
}
