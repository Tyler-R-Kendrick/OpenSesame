...
export function canonicalizeOrigin(
  ...
  const url = parseOrigin(input);
  validateOriginComponents(url);
  ...
  validateOriginTransport(scheme, hostname, options);
  return formatOrigin(scheme, hostname, url.port);
}
...
function validateOriginTransport(
  scheme: string,
  hostname: string,
  options: CanonicalizeOriginOptions,
): void {
...
function formatOrigin(scheme: string, hostname: string, port: string): string {
  const defaultPort =
...
function parseOrigin(input: string): URL {
...
function validateOriginComponents(url: URL): void {
