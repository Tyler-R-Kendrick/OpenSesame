...
describe("domain invariants", () => {
  it("treats provisional as principal state, not a separate type", () => {
    ...
    const upgraded = elevatePrincipalAssurance(p, "verified", "active", fixtures.now);
  ...
  it("rejects principal id mutation", () => {
    const p = fixtures.provisionalPrincipal();
    expect(() =>
      assertPrincipalIdStable(p, { ...p, id: "prn_other" }),
    ).toThrow(DomainError);
  });
  ...
  it("keeps claim ≠ device authorization", () => {
    ...
    expect(() => assertNotSameAsDeviceAuth(session, device)).toThrow(DomainError);
    expect(() =>
  ...
  it("enforces partial claim dependency closure", () => {
    ...
    expect(() => assertDependencyClosure(items, new Set())).toThrow(DomainError);
  });
});
