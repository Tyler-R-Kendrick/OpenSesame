...
const IDENTITY_API = { class: "external-service", purpose: "the configured Identity API", automatic: false } as const;

export const FIXTURE_CATALOG: CapabilityCatalog = buildCatalog(
  [
    describe("vault.passwords", "Passwords", "Items, the editor, login and note kinds, health.", {
      tier: "core",
      operationIds: ["pages.items.list", "pages.items.edit"],
      itemKinds: ["login", "note"],
      keyAccess: "item-plaintext",
    }),
    describe("settings.core", "Settings", "General, Security, Vaults, Danger, Capabilities.", {
      tier: "core",
      operationIds: ["pages.settings.prefs.edit"],
    }),
    describe("vault.passkey-records", "Passkey records", "Store and use passkeys as vault items.", {
      operationIds: ["pages.items.passkey.create"],
      itemKinds: ["passkey"],
      browserPermissions: ["webauthn"],
      keyAccess: "item-plaintext",
    }),
    describe("identity.federation", "Federated sign-in", "Sign in through the configured Identity API.", {
      operationIds: ["pages.identity.signin"],
      egress: [IDENTITY_API],
      requiresService: true,
      offlineLimits: "Sign-in needs the network; a held session keeps working.",
    }),
    describe("access.authority", "Access authority", "Grants, principals and shares.", {
      dependencies: ["identity.federation"],
      operationIds: ["pages.access.grants.list"],
      requiresService: true,
    }),
    describe("connectors.external", "External connectors", "Connectors by reference from a directory.", {
      dependencies: ["access.authority"],
      operationIds: ["pages.connectors.list"],
      egress: [{ class: "external-service", purpose: "the connector directory", automatic: false }],
      requiresService: true,
    }),
    describe("sharing.local-transport", "Local sharing transport", "Share with nearby devices.", {
      environments: ["document", "dedicated-worker"],
      egress: [{ class: "peer-or-local-network", purpose: "nearby devices", automatic: false }],
    }),
    describe("sharing.drops", "Drop sharing", "Share through Identity API drops.", {
      dependencies: ["identity.federation"],
      egress: [{ class: "external-service", purpose: "drop polling against the Identity API", automatic: true }],
      requiresService: true,
    }),
    describe("sharing.household", "Household sharing", "Share items with a household over one transport.", {
      alternatives: [{ slot: "transport", oneOf: ["sharing.local-transport", "sharing.drops"] }],
      operationIds: ["pages.sharing.share"],
    }),
    describe("notifications.web-push", "Web push", "Approval prompts delivered as push notifications.", {
    ...
    describe("support.local-ai", "On-device support model", "Guided help computed on this device.", {
      environments: ["shared-worker"],
      offlineLimits: "Runs entirely on device.",
    }),
    describe("telemetry.external", "External telemetry", "Usage metrics sent to a collector.", {
      egress: [{ class: "external-service", purpose: "the telemetry collector", automatic: true }],
    }),
  ],
  1,
);
...
const UPDATES = { unknownCapabilities: "deny", expandedExposure: "require-approval" } as const;
...
export const FIXTURE_POLICIES: {
  ...
  family: {
    ...
    network: { externalServices: "allow", allowedServiceOrigins: ["https://id.example.test"] },
    updates: UPDATES,
  },
  managedProhibited: {
    schemaVersion: 1,
    ...
    capabilities: {
      default: "deny",
      required: ["identity.federation"],
      optional: ["access.authority", "connectors.external", "vault.passkey-records"],
      prohibited: [
        "sharing.drops",
        "sharing.household",
        "sharing.local-transport",
...
export function fixtureResolveInput(overrides: Partial<ResolveInput> = {}): ResolveInput {
  return {
