...
describe("parseInstancePolicy", () => {
  it("round-trips the fixture policies", () => {
    for (const policy of [FIXTURE_POLICIES.family, FIXTURE_POLICIES.managedProhibited]) {
  ...
  it("MODEL-02: an id in two of required/optional/prohibited fails with the later path", () => {
    ...
    expect(found).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ code: "CONFLICTING_SETS", path: "capabilities.optional[1]" }),
        expect.objectContaining({ code: "CONFLICTING_SETS", path: "capabilities.prohibited[0]" }),
      ]),
    );
  });
  ...
  it("rejects a non-deny default, a foreign updates block, and unknown fields", () => {
    ...
    expect(diagnostics(parseInstancePolicy({ ...base, network: { externalServices: "allow", allowedServiceOrigins: ["https://a", "https://a"] } })).map((d) => d.code)).toContain("DUPLICATE_ID");
  });

  it("bounds ids, revisions and list sizes", () => {
    const base = json(FIXTURE_POLICIES.family);
    expect(diagnostics(parseInstancePolicy({ ...base, revision: "" })).map((d) => d.path)).toContain("revision");
    expect(diagnostics(parseInstancePolicy({ ...base, revision: "r".repeat(65) })).map((d) => d.path)).toContain("revision");
    expect(diagnostics(parseInstancePolicy({ ...base, instanceId: "bad id" })).map((d) => d.code)).toContain("INVALID_ID");
    const tooMany = Array.from({ length: 257 }, (_, i) => `a.c${i}`);
    expect(diagnostics(parseInstancePolicy({ ...base, capabilities: { default: "deny", required: [], optional: tooMany, prohibited: [] } })).map((d) => d.code)).toContain("TOO_MANY_ITEMS");
    expect(diagnostics(parseInstancePolicy({ ...base, capabilities: { default: "deny", required: [], optional: ["a.b", "a.b"], prohibited: [] } })).map((d) => d.path)).toContain("capabilities.optional[1]");
  });
});
...
describe("parseWorkspaceRestriction", () => {
  ...
  it("MODEL-03: allow: null inherits the ceiling; allow: [] permits no optional capability", () => {
    ...
    expect(none.capabilities["connectors.external"]?.reasons).toContain("DENIED_BY_WORKSPACE");
    expect(none.approvedCapabilities).toEqual(["settings.core", "vault.passwords"]);
  });

  it("rejects an id in both allow and prohibited", () => {
    const result = parseWorkspaceRestriction({ ...base, allow: ["a.b"], prohibited: ["a.b"] });
    expect(diagnostics(result)).toEqual([
      expect.objectContaining({ code: "CONFLICTING_SETS", path: "prohibited[0]" }),
    ]);
  });
});
...
describe("selection, vault, receipt and distribution parsers", () => {
  it("round-trip their fixtures", () => {
    ...
    const plan = resolveComposition(
      fixtureResolveInput({ instancePolicy: FIXTURE_POLICIES.family, installation: FIXTURE_INSTALLATION }),
    );
    const receipt = buildConsentReceipt(plan, FIXTURE_CATALOG, "2026-09-22T00:00:00.000Z");
  ...
  it("rejects a receipt whose digest, roots or exposure are inconsistent", () => {
    ...
    const receipt = buildConsentReceipt(plan, FIXTURE_CATALOG, "2026-09-22T00:00:00.000Z");
    expect(diagnostics(parseConsentReceipt({ ...json(receipt), acceptedAt: "2026-09-23T00:00:00.000Z" })).map((d) => d.code)).toContain("INVALID_DIGEST");
    expect(diagnostics(parseConsentReceipt({ ...json(receipt), roots: [...receipt.roots, "telemetry.external"] })).map((d) => d.path)).toContain(`roots[${receipt.roots.length}]`);
    expect(diagnostics(parseConsentReceipt({ ...json(receipt), exposure: { ...receipt.exposure, "a.b": "nope" } })).map((d) => d.path)).toContain("exposure.a.b");
  });

  it("rejects selections with overlapping sets, bad slots and bad choices", () => {
    const base = json(FIXTURE_INSTALLATION);
    expect(diagnostics(parseInstallationSelection({ ...base, selectedOptional: ["identity.federation"] })).map((d) => d.path)).toContain("selectedOptional[0]");
    expect(diagnostics(parseInstallationSelection({ ...base, chosenAlternatives: { Transport: "sharing.drops" } })).map((d) => d.path)).toContain("chosenAlternatives.Transport");
    expect(diagnostics(parseInstallationSelection({ ...base, chosenAlternatives: { transport: "Sharing" } })).map((d) => d.path)).toContain("chosenAlternatives.transport");
    expect(diagnostics(parseInstallationSelection({ ...base, delivery: { prefetch: "all", offlineCache: "shell-only" } })).map((d) => d.path)).toContain("delivery.prefetch");
  });
