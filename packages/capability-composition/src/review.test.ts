...
describe("reviewCompositionChange", () => {
  it("reports what a widening enables: capabilities, modules, operations, egress, permissions, worker", () => {
    ...
    expect(review.addedOperations).toEqual([
      "pages.access.grants.list",
      "pages.connectors.list",
      "pages.items.passkey.create",
    ]);
    expect(review.addedEgress).toEqual([
      {
        automatic: false,
  ...
  it("widened: a tightening that adds nothing is not widened; one that sneaks an approval in is", () => {
    const wide = consented(
      familyInput(["connectors.external", "vault.passkey-records"]),
    );
    ...
    expect(swapped.disabled).toEqual([
      "access.authority",
      "connectors.external",
      "vault.passkey-records",
    ]);
    expect(reviewCompositionChange(wide, wide, FIXTURE_CATALOG).widened).toBe(
      false,
    );
  });
  ...
  it("carries the after-plan's conflicts, consent, restart set and artifact need", () => {
    ...
    const after = resolveComposition({
      ...afterInput,
      receipt: buildConsentReceipt(before, FIXTURE_CATALOG, NOW),
    });
    const review = reviewCompositionChange(before, after, FIXTURE_CATALOG);
    expect(review.restartRequiredFor).toEqual([
      "access.authority/runtime",
      "connectors.external/runtime",
    ]);
    ...
    expect(review.conflicts.map((c) => c.code)).toEqual([
