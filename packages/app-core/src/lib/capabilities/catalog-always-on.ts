...
export const ALWAYS_ON_DESCRIPTORS: readonly AuthoredDescriptor[] = [
  ...
  alwaysOn(
    "identity.ceremonies",
    "Ceremonies",
    "The routes a link opens on this origin (ADR 0140): device sign-in approval at /device and the older links to it; claims and drops at /claim; a cross-device approval at /i/<ref> and a request review at /approve/<ref>, each decided with a passkey touch bound to it; the app hand-off at /invoke/<kind>; and /guest and /delegate, other names for the guest road and a Join invite. They open before unlock and never read the vault.",
    {
      operationIds: [
