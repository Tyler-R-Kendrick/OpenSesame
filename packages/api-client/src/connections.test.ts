...
describe("api-client connections", () => {
  ...
  it("parses the provider catalog", async () => {
    ...
    expect(calls[0]?.url).toBe("http://host.test:8787/api/v1/providers");
  });
  ...
  it("posts create, authorize, credential and binding bodies", async () => {
    ...
    expect(calls[1]?.url).toBe(
      "http://host.test:8787/api/v1/connections/connection_01J/authorize",
    );
  ...
  it("refreshes, revokes, unbinds and reads events", async () => {
    ...
    expect(calls[2]?.url).toBe(
      "http://host.test:8787/api/v1/connections/connection_01J/bindings/binding_01J",
    );
